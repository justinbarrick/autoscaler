...
type doManagerMock struct {
	mock.Mock
}

func (m *doManagerMock) GetNodeGroups() ([]cloudprovider.NodeGroup) {
	args := m.Called()
	return args.Get(0).([]cloudprovider.NodeGroup)
}

func (m *doManagerMock) SetURL(string) {
	m.Called()
}

func TestBuildDOCloudProvider(t *testing.T) {
	doManagerMock := &doManagerMock{}

	resourceLimiter := cloudprovider.NewResourceLimiter(
		map[string]int64{cloudprovider.ResourceNameCores: 1, cloudprovider.ResourceNameMemory: 10000000},
		map[string]int64{cloudprovider.ResourceNameCores: 10, cloudprovider.ResourceNameMemory: 100000000})

	provider, err := BuildDOCloudProvider(doManagerMock, resourceLimiter)
...
func TestNodeGroups(t *testing.T) {
	doManagerMock := &doManagerMock{}
	do := &DOCloudProvider{
		doManager: doManagerMock,
	}
	doManagerMock.On("GetNodeGroups").Return([]cloudprovider.NodeGroup{}).Once()
	result := do.NodeGroups()
	assert.Equal(t, []cloudprovider.NodeGroup{}, result)
	mock.AssertExpectationsForObjects(t, doManagerMock)
}
